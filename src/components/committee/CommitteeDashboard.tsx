...
export default function CommitteeDashboard() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/committee/workflow?action=${activeTab}`);
      const data = await response.json();

      if (data.success) {
        setApplications(data.applications || data.approvals || []);
      } else {
  ...
  const getStatusBadge = (application: Application) => {
    if (application.committeeReviewed) {
      if (application.committeeStatus === 'approved') {
        return <Badge className="bg-green-100 text-green-800">อนุมัติแล้ว</Badge>;
      } else {
        return <Badge className="bg-red-100 text-red-800">ปฏิเสธแล้ว</Badge>;
      }
    } else if (application.committeeReceived) {
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={setActiveTab}>
        ...
        <TabsContent value="approvals">
          <Card>
            <CardHeader>
              <CardTitle>รายการการอนุมัติ</CardTitle>
            </CardHeader>
            <CardContent>
              {renderApplicationsTable()}
            </CardContent>
