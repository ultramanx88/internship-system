...
interface CommitteeWorkflowActionsProps {
  applicationId: string;
  workflowStatus: any;
  onActionComplete?: () => void;
}

export default function CommitteeWorkflowActions({ 
  applicationId, 
  workflowStatus, 
  onActionComplete 
}: CommitteeWorkflowActionsProps) {
  ...
  const handleSubmit = async () => {
    ...
    setLoading(true);
    try {
      const response = await fetch('/api/committee/workflow/actions', {
        method: 'POST',
        headers: {
  ...
  const getActionButton = (action: 'receive_application' | 'review_application', status?: 'approved' | 'rejected') => {
    const actions = {
      receive_application: {
        ...
        disabled: workflowStatus?.committeeReceived || false
      },
      review_application: {
        ...
        disabled: !workflowStatus?.committeeReceived || workflowStatus?.committeeReviewed || false
      }
    };
  ...
  const getActionDescription = (action: string) => {
    switch (action) {
      case 'receive_application':
        return 'ยืนยันการรับคำขอฝึกงานหลังจากอาจารย์ประจำวิชาอนุมัติ';
      case 'review_application':
        return reviewStatus === 'approved' 
          ? 'อนุมัติคำขอฝึกงาน'
          : 'ปฏิเสธคำขอฝึกงานและระบุเหตุผล';
      default:
  ...
  return (
    <div className="space-y-4">
      <h3 className="text-lg font-semibold">การดำเนินการ Committee Workflow</h3>
