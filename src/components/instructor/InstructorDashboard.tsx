...
import InstructorWorkflowStatus from './InstructorWorkflowStatus';
import InstructorWorkflowActions from './InstructorWorkflowActions';

interface Application {
  id: string;
  dateApplied: string;
  projectTopic?: string;
  courseInstructorStatus?: string;
...
export default function InstructorDashboard() {
  ...
  const fetchApplications = async () => {
    try {
      setLoading(true);
      const response = await fetch(`/api/instructor/workflow?action=${activeTab}`);
  ...
  const getStatusBadge = (application: Application) => {
    if (application.supervisorAssigned) {
    ...
    } else if (application.instructorReceived) {
      return <Badge className="bg-yellow-100 text-yellow-800">รอพิจารณา</Badge>;
    } else {
      return <Badge className="bg-gray-100 text-gray-800">รอรับคำขอ</Badge>;
    }
  };
